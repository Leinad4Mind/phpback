...
<div class="max-w-md mx-auto py-12">
    <div class="bg-card text-card-foreground rounded-lg border shadow-sm p-8 text-center">
        ...
        <h2 class="text-2xl font-bold tracking-tight mb-6"><?= esc($lang['label_admin_panel'] ?? 'Admin Panel') ?></h2>
        ...
        <?php if ($error === 'noadmin'): ?>
            <div class="bg-destructive/10 text-destructive border border-destructive/20 p-4 rounded-md mb-6 font-medium">
                <?= esc($lang['error_noadmin'] ?? 'You are not an admin') ?>
            </div>
        <?php else: ?>
            <?php if ($error === 'error'): ?>
                <div class="bg-destructive/10 text-destructive border border-destructive/20 p-3 rounded-md mb-6 text-sm font-medium">
                    <?= esc($lang['error_login'] ?? 'Email or password are incorrect') ?>
                </div>
            <?php elseif ($error === 'toomany'): ?>
                <div class="bg-destructive/10 text-destructive border border-destructive/20 p-3 rounded-md mb-6 text-sm font-medium">
                    <?= esc($lang['error_toomany'] ?? 'Too many attempts. Please try again later.') ?>
                </div>
            <?php endif; ?>
            ...
            <form action="<?= base_url('adminaction/login') ?>" method="POST" class="space-y-4">
                ...
                <div class="relative">
                    ...
                    <input type="email" name="email" class="flex h-10 w-full rounded-md border border-input bg-background pl-10 pr-3 py-2 text-sm ring-offset-background placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2" placeholder="<?= esc($lang['label_email'] ?? 'Email', 'attr') ?>" required>
                </div>
                ...
                <div class="relative">
                    ...
                    <input type="password" name="password" class="flex h-10 w-full rounded-md border border-input bg-background pl-10 pr-3 py-2 text-sm ring-offset-background placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2" placeholder="<?= esc($lang['label_password'] ?? 'Password', 'attr') ?>" required>
                </div>
