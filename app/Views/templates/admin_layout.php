            ...
            <div class="flex items-center justify-between h-16">
                ...
                <div class="flex items-center gap-8">
                    <a href="<?= base_url('admin/dashboard') ?>" class="flex items-center gap-3 hover:opacity-90">
                        ...
                        <span
                            class="font-bold tracking-tight"><?= esc($lang['label_admin_panel'] ?? 'Admin Panel') ?></span>
                    </a>

                    <nav class="hidden md:flex items-center gap-1">
                        <a href="<?= base_url('admin/dashboard') ?>"
                            class="px-3 py-2 rounded-md text-sm font-medium <?= ($active === 'dashboard') ? 'bg-slate-800 text-white' : 'text-slate-300 hover:bg-slate-800 hover:text-white transition-colors' ?>"><?= esc($lang['label_dashboard'] ?? 'Dashboard') ?></a>

                        <a href="<?= base_url('admin/ideas') ?>"
                            class="px-3 py-2 rounded-md text-sm font-medium <?= ($active === 'ideas') ? 'bg-slate-800 text-white' : 'text-slate-300 hover:bg-slate-800 hover:text-white transition-colors' ?>"><?= esc($lang['label_ideas_comments'] ?? 'Ideas and Comments') ?></a>

                        <?php if (is_admin(2)): ?>
                            <a href="<?= base_url('admin/users') ?>"
                                class="px-3 py-2 rounded-md text-sm font-medium <?= ($active === 'users') ? 'bg-slate-800 text-white' : 'text-slate-300 hover:bg-slate-800 hover:text-white transition-colors' ?>"><?= esc($lang['label_users_management'] ?? 'Users Management') ?></a>
                        <?php endif; ?>

                        <?php if (is_admin(3)): ?>
                            <a href="<?= base_url('admin/system') ?>"
                                class="px-3 py-2 rounded-md text-sm font-medium <?= ($active === 'system') ? 'bg-slate-800 text-white' : 'text-slate-300 hover:bg-slate-800 hover:text-white transition-colors' ?>"><?= esc($lang['label_system_settings'] ?? 'System Settings') ?></a>
                        <?php endif; ?>
